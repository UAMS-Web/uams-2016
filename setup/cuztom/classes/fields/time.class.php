...
class Cuztom_Field_Time extends Cuztom_Field
{
	...
	public $css_classes     = array( 'js-cuztom-timepicker', 'cuztom-timepicker', 'timepicker', 'cuztom-input' );
	...
	public function _output( $value, $object = null )
	{
		...
		if ( ! empty( $value ) ) {
			...
			if ( false !== $timestamp ) {
				$format   = isset( $this->args['time_format'] ) ? $this->args['time_format'] : 'H:i';
				$time_val = date( $format, $timestamp );
			}
		}

		$display_val = ! empty( $time_val ) ? $time_val : $this->default_value;
